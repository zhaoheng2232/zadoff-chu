function [detected,cfoEstHz,tauEst] = init_sync_v2(fadingChan,chanFlag,nodeCfg,frameCfg,algCfg,syncSeqSet,plDbMat,propDelayMat)
...
for indxRx = 1:numLink
    ...
    for indxTx = 1:numLink
        ...
        for indxMp = 1:numMultipath
            ...
            h1 = sqrt(maxTxPowLin/numTxAnte*pathlossLin(indxRx,indxTx))*squeeze(sum(Hij(:,1:numTxAnte/2,:),2));
            h2 = sqrt(maxTxPowLin/numTxAnte*pathlossLin(indxRx,indxTx))*squeeze(sum(Hij(:,numTxAnte/2+1:numTxAnte,:),2));
            seqTmp1 = [zc1(end-syncSeqCpLen+1:end) zc1];
            seqTmp2 = [zc2(end-syncSeqCpLen+1:end) zc2];
            Y(:,dly+1:dly+epochLen) = Y(:,dly+1:dly+epochLen)+h1.*repmat(seqTmp1,numRxAnte,1)+h2.*repmat(seqTmp2,numRxAnte,1);
        end
    end
    
    %--- estimate zeta
    start = syncSeqCpLen; %+8 because the pulse shaping introduce a 8-sample delay
    zc1 = syncSeqSet(indxRx,:);
	zc2 = conj(zc1);
    ...
    Y2 = Y(:,start+1:start+syncSeqLen).*(ones(numRxAnte,1)*conj(zc2));
    ...
    tauEst(indxRx) = (zeta2-zeta1)*syncSeqLen/indxRx/2;
end
% fprintf('---------------\n');
